...
export default function Contact() {
  ...
  const items = [
    ...
    contact.whatsapp && {
      icon: <IconWhatsApp className="h-5 w-5 shrink-0" />,
      label: 'WhatsApp',
      content: (
        <a
    ...
    contact.phone && {
      ...
      content: (
        <a
          href={`tel:+34${contact.phone.replace(/\s/g, '')}`}
    ...
    contact.email && {
      ...
      content: (
        <div className="relative inline-block">
          ...
          <span
            className={`pointer-events-none absolute -top-8 left-0 whitespace-nowrap rounded bg-primary px-2 py-1 text-xs text-background transition-all duration-200 ${emailCopied ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'}`}
          >
          ...
          <dl className="divide-y divide-primary/10 border-t border-primary/10">
            {items.map((item, i) => (
              <div key={i} className="flex items-center gap-5 py-5">
                ...
                <dt className="w-24 shrink-0 text-xs uppercase tracking-[0.2em] text-secondary">
                  {item.label}
                </dt>
