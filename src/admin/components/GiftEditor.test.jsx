...
describe('GiftEditor — сколько штук', () => {
  ...
  // Соотношение и размер — то, что нужно знать до того, как открыл редактор
  // фотографий, а не после загрузки не той картинки.
  it('называет соотношение и размер прямо под полем', () => {
    renderEditor({ value: rule });
    expect(screen.getByText(/Соотношение 4:5, рекомендуем 800 × 1000 px/)).toBeTruthy();
  });
