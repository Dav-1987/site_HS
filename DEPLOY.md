...
| **ffmpeg** | нормализация загруженных видео (`server/video.js`); ставится `apt-get install --no-install-recommends ffmpeg` |
...
### Нормализация видео (ffmpeg)

Телефон пишет **HEVC**, а браузеры тянут его неравномерно: Safari играет всегда,
Chrome только при аппаратном декодере, Firefox долго не умел вовсе — у части
посетителей вместо ролика чёрный прямоугольник, и в аналитике это не видно.
Плюс сырая запись приезжает в 1440×2560 или 4K там, где рамка шириной 600 px.

Поэтому загруженные видео приводятся к **H.264** и ужимаются до 720 px по
ширине (портрет) / 1280 px (ландшафт) — `server/video.js`.

- **Когда:** проход запускается по кнопке «Пересобрать сайт»
  (`POST /api/admin/rebuild`), а не при загрузке — на одноядерном VPS
  кодирование в момент загрузки отобрало бы процессор у Express и Postgres.
  Залитое видео до ближайшей пересборки отдаётся как есть.
- **Идемпотентность:** уже нормализованные файлы отсеиваются по `ffprobe`,
  повторный проход по всему каталогу стоит ~3 секунды.
- **Приоритет:** ffmpeg вызывается через `nice -n 19` и `-threads 1`, чтобы
  запрос к сайту всегда получал процессор раньше кодировщика.
- **Замена на месте:** имя файла не меняется (URL — это sha256 исходных байт,
  он лежит в БД). Результат пишется во временный файл и переименовывается
  поверх только после проверки — оборванная кодировка не оставит битый ролик.
- ⚠️ **Поворот.** Телефон часто пишет кадр «как есть» плюс матрицу поворота,
  поэтому размеры из контейнера могут быть перевёрнуты относительно того, что
  увидит браузер. `parseProbe` учитывает это; при правках не переходить обратно
  на «сырые» `width`/`height`, иначе портрет получит ландшафтный лимит.
- **Звук** сохраняется, если дорожка не тишина (порог −50 дБ): на странице
  товара и в лайтбоксе видео играет с элементами управления, то есть со звуком.

Разовая пакетная обработка (например, после переезда) — тем же модулем:

```bash
ssh root@185.202.172.59 "cd /var/www/hs-muebles/server && node --input-type=module -e \"
import { readdirSync } from 'node:fs';
import { isVideoFile, sweepVideos } from '/var/www/hs-muebles/server/video.js';
const dir = '/var/www/hs-muebles/uploads';
console.log(await sweepVideos(dir, readdirSync(dir).filter(isVideoFile)));
\""
```
